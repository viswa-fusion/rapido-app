...
class RideDatabase : RideDbDao {
    ...
    override fun insertRide(ride: Ride, passengerId: Int): DbResponse {
        query =
            "insert into ${DbTables.rides.name} " +
                    "(`passenger_id`,`pickup_location`,`drop_location`,`status`,`total_charge`)value(?,?,?,?,?)"
        connection.prepareStatement(query).use {
            ...
            if (result == 0) {
                return DbResponse.OperationUnsuccessful(
                    "unable to add $ride into ${DbTables.rides.name} in insert ride function"
                )
            }
    ...
    override fun checkUserHaveRide(loggedUserid: Int): Boolean {
        query =
            "SELECT * FROM ${DbTables.rides} " +
                    "WHERE passenger_id = '$loggedUserid' " +
                    "AND status NOT IN ('${RideStatus.COMPLETED.name}', '${RideStatus.CANCEL.name}')"
    ...
    override fun updateRide(rideId: Int, ride: Ride, passengerId: Int, DriverId: Int?): DbResponse {
        query =
